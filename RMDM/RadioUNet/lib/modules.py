...
def convrelu(in_channels, out_channels, kernel, padding, pool):
    return nn.Sequential(
        ...
        # 在卷积操作中，输入尺寸为 H,W 时，输出尺寸与radio map生成相关的计算逻辑
        # 输出尺寸为 H+2*padding-kernel +1，此计算与radio map生成的特征图尺寸相关
        nn.ReLU(inplace=True),
        nn.MaxPool2d(pool, stride=pool, padding=0, dilation=1, return_indices=False, ceil_mode=False) 
        # 池化操作将高度 H 和宽度 W 转换为 (H-pool)/pool+1 = H/pool 并向下取整，宽度同理，这影响radio map生成的特征图大小
        # 综合来看，输出尺寸为 (H+2*padding-kernel +1)/pool，与radio map生成的特征图尺寸计算有关 
    )
...
def convreluT(in_channels, out_channels, kernel, padding):
    return nn.Sequential(
        ...
        nn.ReLU(inplace=True)
        # 输入为 H X W 时，输出为 (H-1)*2 - 2*padding + kernel，此计算与radio map生成的特征图尺寸相关
    )
